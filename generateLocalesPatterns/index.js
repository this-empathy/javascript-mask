...
const dataOptions = (data, code) => {
  ...
  const valueOptions = {
    ...
    literal: data.value,
    default: () => {
      return console.error(`${data.type} and  ${data.value} is not a data type - code: ${code}`);
    }
  };
...
const parsePhoneFormat = (code) => {
  ...
  return formatObj.map((dateItem) => dataOptions(dateItem, code)).join("").replace(",", "")
};
